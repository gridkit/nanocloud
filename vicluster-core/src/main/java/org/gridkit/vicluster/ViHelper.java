...
public class ViHelper {
     ...
     * Will try to load configuration using provided filename, first from classpath, then from file system.
     */
    public static void configure(ViNodeSet manager, String filename) throws IOException {
        InputStream is = Thread.currentThread().getContextClassLoader().getResourceAsStream(filename);
        if (is == null) {
            is = new FileInputStream(filename);
        }
    ...
    @SuppressWarnings("serial")
    public static void configure(final ViNodeSet manager, InputStream reader) throws IOException {
        new Properties() {
            @Override
            public  Object put(Object key, Object value) {
                String skey = (String) key;
    ...
    public static void configure(final ViNodeSet manager, Iterable<Map.Entry<String, String>> props) {
        for(Map.Entry<String, String> prop : props) {
            String skey = prop.getKey();
            String svalue = prop.getValue();
            int pi = svalue.indexOf("=");
