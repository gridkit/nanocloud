...
public class ExceptionHelper {
    ...
    public static <T extends Throwable> T addStackElementsAtBottom(T t, StackTraceElement insertAtMiddle, StackTraceElement[] addToBottom){
        final StackTraceElement probablyGetStackTraceElement = addToBottom[0];
        if (Thread.class.getName().equals(probablyGetStackTraceElement.getClassName()) && "getStackTrace".equals(probablyGetStackTraceElement.getMethodName())){
            StackTraceElement[] newAddToBottom = new StackTraceElement[addToBottom.length-1];
            System.arraycopy(addToBottom, 1, newAddToBottom, 0, newAddToBottom.length);
            addToBottom = newAddToBottom;
        }
        final StackTraceElement[] stackTraceFromException = t.getStackTrace();
        final StackTraceElement[] stackTrace = new StackTraceElement[addToBottom.length+stackTraceFromException.length+1];
        System.arraycopy(stackTraceFromException, 0, stackTrace, 0, stackTraceFromException.length);
        stackTrace[stackTraceFromException.length] = insertAtMiddle;
        System.arraycopy(addToBottom, 0, stackTrace, stackTraceFromException.length + 1, addToBottom.length);
